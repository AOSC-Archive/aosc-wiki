...
- [ ] AOSC Packages (site): Current packages.aosc.io JSON does not detect version bumps when its parent link changes (for instance, [https://download.gnome.org/sources/gnome-shell/3.24](https://download.gnome.org/sources/gnome-shell/3.24) versus [https://download.gnome.org/sources/gnome-shell/3.30](https://download.gnome.org/sources/gnome-shell/3.30).
    - [ ] JSON is also unreliable with `foo-n` (`n` is a number) package names, as it cuts off at the last `-`.
...
- [x] Autobuild3: Reliability issues with build scripts.
...
- [x] Build result feedback.
   - [x] Autobuild3/ACBS/AOINB/Ciel: Package builds failed/succeeded (Since Autobuild3 `v20200320` and Ciel `v2.4.4`).
...
- [ ] ACBS/AOINB: Modularized solution. One tool for one function.
    - [x] Metadata: `abbs-meta`, `packages-site/dpkgrepo.py.`
    - [ ] Move dpkgrepo.py to abbs-meta?
    - [x] Source downloader: `adbs/downloader.py`.
    - [ ] Dependency resolver and topology sorting: [abbs-dep](https://github.com/AOSC-Dev/abbs-dep).
        ...
        - [x] Topology sorting, without checking repository version.
    - [ ] Build environment manager(s).
          - [x] Container: [Ciel](https://github.com/AOSC-Dev/ciel).
...
- [ ] Quality Assurance.
    ...
    - [x] Check for problems on library-level: `bigcat-dbg`.
...
- [ ] QA in Autobuild3 as a standardised component (or enhancing the current implementation).
    ...
    - [x] Custom/non-standard DPKG sections, dangling files, incorrect paths (`/usr/local` in distribution packages, etc.).
